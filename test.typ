...
#show: springer-spaniel.template(
  title: [Intrusion Detection System using LAD],
  authors: (
    (
      name: "Bhaumikaditya Guleria",
      institute: "NIT Uttrakhand",
  ...
  abstract:  [
Intrusion Detection Systems (IDS) will continue to play a critical role in safe-
guarding networks from malicious activities in an increasingly connected world.
Traditional IDS approaches are expected to face persistent challenges, including
high false positive rates, inefficiency in handling evolving threats, and limited
adaptability to diverse data distributions. This research will investigate the
potential of Logical Analysis of Data (LAD) as a robust mathematical frame-
work for enhancing intrusion detection. LAD will be employed to identify critical
patterns in network traffic, leveraging its ability to handle heterogeneous and
imbalanced datasets effectively. The study will design a LAD-based IDS model
and evaluate its performance on widely recognized datasets, such as NSL-KDD
and KDD-Cup99. Key performance metrics, including accuracy, precision, recall,
F2-score, and data coverage, will be assessed to measure the effectiveness of the
proposed model. It is anticipated that the LAD-based approach will demonstrate
superior adaptability and accuracy compared to traditional methods, while also
addressing issues of dataset coverage and computational efficiency. The findings
of this study will provide valuable insights into the application of LAD in cyber-
security and will pave the way for the development of more robust, scalable, and
adaptive IDS models. Future research will focus on refining the LAD methodol-
ogy to address its limitations and further enhance its applicability to real-world
...
The remainder of this paper is organized as follows: Section~#link(<sec:Review>)[2] provides an overview of related work in IDS and LAD. Section~#link(<sec:Methodology>)[3] describes the experimental setup and LAD-based rule generation process. Section~#link(<sec:Results>)[4] presents the results of our experiments, followed by an analysis of their implications. Finally, Section~#link(<sec:Conclusion>)[5] concludes the study with a discussion of future research directions.
...
placement: auto,
table(
  columns: (auto, 2fr, 5fr),
  align: horizon,
  table.header(
[#strong[Category]], [#strong[Focus Area]], [#strong[Key Findings / Contributions]]),
table.cell(rowspan: 2)[#strong[IDS: Signature-based]], table.cell(rowspan: 2)[ Pattern recognition],[ Performance evaluation of signature-based IDS #cite(<IDS4>);],
[Deep recurrent neural networks for improved signature generation #cite(<IDS14>);],
table.cell(rowspan: 3)[#strong[IDS: Anomaly-based]],[ Feature extraction],[ Deep learning-based feature extraction mechanism #cite(<IDS1>);],
...
[ IoT intrusion detection],[ LAD and information gain ratio for IoT intrusion detection #cite(<IDS15>);],
 table.cell(rowspan: 3)[#strong[IDS: Hybrid Approaches]],[ Enhanced detection],[ Integration of anomaly and misuse detection for improved performance #cite(<IDS6>);#cite(<IDS8>);],
[ IoT security],[ Hybrid IDS for scalable and secure IoT environments #cite(<IDS5>);],
[ Surveys],[ Comprehensive surveys addressing challenges in machine learning-based IDS #cite(<IDS2>);#cite(<IDS19>);],
table.cell(rowspan: 2)[#strong[LAD: Foundations]],[ Pattern discovery],[ Introduction to LAD for analyzing numerical data #cite(<LAD1>);#cite(<LAD2>);],
[ Algorithm development],[ Accelerated LAD pattern detection #cite(<LAD5>); Boolean functions for cause-effect analysis #cite(<LAD6>);],
 table.cell(rowspan: 4)[#strong[LAD: Applications]],[ IoT and Security],[ LAD methods applied to IoT intrusion detection #cite(<LAD7>);],
...
#figure(
placement: none,
[#box(width: auto, image("Methedology.drawio.png"));],
  caption: [
    LAD Process Flow
  ]
)
...
#figure(
placement: none,
[#box(width: auto, image("Binarization.drawio.png"));],
  caption: [
...
#figure(
  placement: none,
  [#box(width: 250pt, image("Binning.drawio.png"));],
  caption: [
    Binning Process
  ]
)
...
@tab:binarization demonstrates how
original feature values are sequentially transformed through the steps
of binning, label encoding, and binary conversion, highlighting the
systematic nature of the binarization process and its effectiveness in
...
#figure(
placement: none,
[#box(width: 250pt, image("Pattern.drawio.png"));],
  caption: [
...
@tab:4result provides an overview of the
performance metrics across the datasets NSLKDD and KDDCup99 \(10%). The
metrics include accuracy, precision, recall, F2-score, and coverage. For
the NSLKDD dataset, the accuracy achieved was 75.48%, with a precision
of 85.58%, a recall of 75.48%, an F2-score of 77.30%, and a coverage of
...
@tab:Dataset_Samples provides
insights into the characteristics of the datasets used in the
experiments. The KDDCup99 \(10%) dataset consists of 330,994 samples
with 41 features, 32 of which are preprocessed and 29 are binarized. In
contrast, the NSLKDD dataset comprises 125,973 samples with 41 features,
of which 54 are binarized. These details emphasize the variability in
dataset size and feature representation, highlighting the adaptability
of the binarization and rule-generation process.
...
@tab:Rules_Generated focuses on
the number of rules generated for the datasets based on different
minimum match criteria at a depth of 3. For the KDDCup99 dataset, 243
rules were generated for at least one match, 85 for a minimum of 10
matches, and 42 for a minimum of 50 matches. Similarly, for the NSLKDD
dataset, the number of rules generated was 365, 150, and 65 for minimum
matches of 1, 10, and 50, respectively. These statistics indicate how
stricter matching criteria reduce the number of generated rules,
ensuring more selective and meaningful rules.
  ...
  #figure(
    ...
    caption: [Rules Generated Based on Matches],
  placement: none,
  )<tab:Rules_Generated>,

@tab:Overall_Metrics summarizes
the overall metrics for both datasets under different matching criteria.
For the KDDCup99 dataset, with at least one match, the accuracy,
precision, recall, and coverage were 95.91%, 99.96%, 95.91%, and 95.94%,
respectively. As the match criteria increased to 10 and 50, the accuracy
and recall reduced to 93.27% and 75.80%, respectively, but the precision
remained extremely high \(99.98% and 99.97%). Similarly, for the NSLKDD
dataset, stricter match criteria also led to lower accuracy and recall
while maintaining high precision. For instance, at one match, the
accuracy was 75.42%, which dropped to 71.93% and 70.08% for match
criteria of 10 and 50, respectively.
  ...
  #figure(
    table(
      ...
      table.cell(rowspan: 3)[KDDCup99], [1],[95.91%],[99.96%],[95.91%],[-],[95.94%],
                                       [10],[93.27%],[99.98%],[93.27%],[-],[93.29%],
                                       [50],[75.80%],[99.97%],[75.80%],[-],[75.81%],
      table.cell(rowspan: 3)[NSL-KDD],  [1],[75.42%],[86.13%],[75.42%],[-],[91.52%],
                                       [10],[71.93%],[87.29%],[71.93%],[-],[85.55%],
                                       [50],[70.08%],[88.83%],[70.08%],[-],[81.17%],
    ),
    caption: [Overall Performance Metrics by Matches],
  placement: none,
  )<tab:Overall_Metrics>

The decision matrices provided in @tab:NSLKDD_Matrix and
@tab:KDDcup_Matrix offer a detailed view
of classification performance for the NSLKDD and KDDCup99 datasets under
...
@tab:NSLKDD_Matrix, the number of
correctly classified \"Attack\" and \"Normal\" instances decreases as
the minimum match criterion becomes stricter, resulting in an increase
in #emph[NULL] \(unmatched) cases. For instance, with a minimum of one
...
dataset in @tab:KDDcup_Matrix.
For example, with one match, 124,431 \"DOS\" instances and 30,515
\"Normal\" instances were correctly classified, while with 50 matches,
these numbers dropped to 95,373 and 27,265, respectively. This
illustrates the trade-off between stricter match criteria and
...
to training time and dataset coverage to enhance the overall performance
of the system.
